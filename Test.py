        ...
        for col in range(5):
            for row in range(5):
                ...
                if cell_key not in self.cells:
                    if col == 0 and row == 0:
                        self.cells[cell_key] = Cell(self.window, row, col, "origin", "Title")
                    elif col == 0:
                        self.cells[cell_key] = Cell(self.window, row, col, "row", f'ROW {row}')
                    elif row == 0:
                        self.cells[cell_key] = Cell(self.window, row, col, "col", f'COL {col}')
                    else:
                        self.cells[cell_key] = Cell(self.window, row, col, "data", "0")
        # Start main loop
